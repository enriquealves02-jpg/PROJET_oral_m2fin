...
\subsection{Deux mesures complémentaires}

\noindent\begin{tcolorbox}[colback=blue!5,colframe=blue!50!black,title=\textbf{Important : distinction}]
On peut mesurer le risque de deux façons. Les deux sont valides et complémentaires.

\textbf{1. Mesure dynamique (à chaque CG)} : la mesure ci-dessus. Simule \og que se passerait-il si on appliquait
$T_q$ à chaque CG \fg{}. C'est ce que la démo Streamlit visualise.

\textbf{2. Protocole officiel (1 prédiction par alerte au last CG)} : on émet une prédiction unique par alerte
au format \texttt{predictions.csv} et on compte les éclairs $<3$ km arrivant après la fin prédite. À $q=0{,}95$ :
...
\begin{itemize}[topsep=0pt]
    \item Protocole officiel : \textbf{0,60 \% de risque} sur les éclairs $<3$ km, gain $\sim$ 330 heures.
...
\begin{itemize}[leftmargin=*]
    \item \texttt{Weibull\_final3km.ipynb} : notebook principal d'évaluation 3 km
