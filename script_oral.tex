...
modèle qu'aucun CG ne suive dans les 30 minutes. Puis on fait un sweep sur le seuil $\theta = q$,
en restreignant le risque aux \textbf{CG <3 km} (les IC ne sont pas dangereux au sol). \fg{}
...
\begin{tabular}{ccccc}
\toprule
\textbf{$q = \theta$} & \textbf{Alertes filtrées} & \textbf{CG manqués} & \textbf{Risque CG <3 km} & \textbf{Gain} \\
\midrule
...
Baseline 30 min & — & 0 / 385 & 0{,}00\,\% & 0 h \\
\bottomrule
\end{tabular}
\caption{Protocole officiel, restriction CG <3 km — sweep sur $\theta = q$.}
\end{table}

\textbf{Lecture} :
\og Le baseline 30 min est parfait par construction : aucun CG n'arrive après last\_CG + 30 min
dans une alerte. Le modèle accepte donc un peu de risque pour gagner du temps. À $q = 0{,}99$
on obtient 2,86 \% de risque (11 CG manqués sur 385) pour 26 heures de gain. C'est notre meilleur
compromis sur le critère CG strict. \fg{}

\textbf{Honnêteté scientifique} : la cible officielle $R < 2\%$ n'est pas atteinte avec ce
mapping de confidence — le minimum est 2,86 \%. Pistes : un classifieur secondaire entraîné
sur la cible \og dernier CG \fg{} pour une meilleure confidence, ou un cap dur à 30 min sur
les CG en début d'alerte.
