...
\textbf{2. Protocole officiel oracle (1 prédiction par alerte au last CG ground-truth)} :
on émet une prédiction unique par alerte au format \texttt{predictions.csv} et on compte les
éclairs $<3$ km arrivant après la fin prédite. À $q=0{,}95$ :
risque officiel $= 0{,}60$ \% (12 / 1\,995), gain $\approx 330$ heures.

\textbf{3. Protocole officiel \emph{sweep} (réaliste, sans info de last CG)} :
notebook \texttt{Evaluation\_databattle\_weibull.ipynb}. On émet une prédiction à chaque CG
avec \texttt{confidence}~$= S(30 \mid X)$, puis sweep sur $\theta$.
Meilleur point opérationnel : $\theta = 0{,}95 \Rightarrow R = 3{,}61\%$, gain $= 26$\,h.
Aucun $\theta$ n'atteint $R < 2\%$ avec cette confidence — la marge entre les mesures 2 et 3
quantifie la difficulté d'identifier en temps réel quel CG sera le dernier.
\end{tcolorbox}
...
\subsection{Reproduction fidèle du protocole officiel (sweep $\theta$)}

\textbf{À dire à l'oral} : \og Pour valider notre approche selon la procédure exacte du jury, nous avons
reproduit le notebook officiel dans \texttt{Evaluation\_databattle\_weibull.ipynb}. Une prédiction est
émise à chaque CG avec une confidence définie comme $S(30 \mid X)$ — la probabilité prédite par le
modèle qu'aucun CG ne suive dans les 30 minutes. Puis on fait un sweep sur le seuil $\theta$. \fg{}
...
\begin{tabular}{ccc}
\toprule
\textbf{$\theta$} & \textbf{Risque} & \textbf{Gain} \\
\midrule
0{,}00 & 97{,}59\% & 1\,125\,h \\
0{,}50 & 15{,}24\% &     76\,h \\
0{,}75 &  4{,}06\% &     53\,h \\
...
\bottomrule
\end{tabular}
\caption{Sweep $\theta$ — protocole officiel reproduit sur eval 2023-25.}
\end{table}

\textbf{Lecture} :
\og La courbe est monotone, comme attendu. À $\theta = 0$ on conserve toutes les prédictions et le
risque est très élevé car la prédiction la plus précoce provient du premier CG de l'alerte. En
filtrant sur $\theta$, on ne garde que les CG dont le modèle estime qu'ils sont probablement les
derniers, ce qui fait chuter le risque jusqu'à $3{,}6\%$ à $\theta = 0{,}95$. \fg{}

\textbf{Honnêteté scientifique} : la cible officielle $R < 2\%$ n'est pas atteinte avec ce mapping
de confidence. Le minimum est $3{,}61\%$. La marge entre ce résultat et l'oracle ($0{,}60\%$ à
$q = 0{,}95$ section 8 de \texttt{Weibull\_final3km.ipynb}) mesure la difficulté à identifier en
temps réel quel CG sera le dernier. Une piste d'amélioration : un classifieur secondaire entraîné
sur la cible \og dernier CG \fg{} pour produire une meilleure confidence.
...
éclair un temps d'attente adapté au contexte de l'orage. Sur eval 2023-25, à $q=0{,}95$ :
\begin{itemize}[topsep=0pt]
    \item Protocole officiel oracle (1 pred / alerte au last CG) : \textbf{0,60 \%} de risque,
    gain $\sim$ 330 heures.
    \item Protocole officiel sweep $\theta$ (réaliste, sans info ground-truth) : minimum
    $\mathbf{R = 3{,}61\%}$ à $\theta = 0{,}95$ pour 26 heures de gain.
    \item Mesure dynamique : \textbf{37 incidents visualisables} dans la démo Streamlit.
\end{itemize}
La démo permet d'inspecter chaque incident en détail. Le code, le rapport et la présentation sont fournis. Merci. \fg{}
\end{tcolorbox}
