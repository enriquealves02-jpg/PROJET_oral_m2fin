...
\begin{tabular}{ccc}
\toprule
\textbf{$\theta$} & \textbf{Risque} & \textbf{Gain cumulé} \\
\midrule
0,00 & 97,59\%  & 1\,125\,h \\
0,05 & 41,45\%  &    278\,h \\
0,20 & 19,45\%  &    119\,h \\
...
0,75 &  4,06\%  &     53\,h \\
0,90 &  3,91\%  &     36\,h \\
\textbf{0,95} & \textbf{3,61\%} & \textbf{26\,h} \\
\bottomrule
\end{tabular}
\caption{Sweep sur $\theta$ — protocole officiel reproduit. Le risque descend monotonement de
97,59\% à 3,61\% quand $\theta$ croît.}
\label{tab:sweep_theta}
\end{table}

\paragraph{Interprétation.} La forme attendue est obtenue : à mesure que $\theta$ croît, on filtre
les prédictions provenant de CG en milieu d'alerte (faible $S(30|X)$) et on ne garde que celles
des CG terminaux. Le risque s'effondre (de 97,59\% à 3,61\%) tandis que le gain décroît en parallèle
(1\,125\,h à 26\,h).
...
$R = 3{,}61\%$ à $\theta = 0{,}95$ pour un gain de 26 heures.

Cela tient au fait que \texttt{confidence} $= S(30|X)$ est un signal \emph{utile mais imparfait}
pour identifier les CG terminaux. Quelques CG en milieu d'alerte ont une survie élevée par hasard
(orages denses où on aurait dû attendre).

\textbf{À titre comparatif}, en exploitant l'information ground-truth \texttt{is\_last == True}
(une seule prédiction par alerte au dernier CG observé), on obtient à $q = 0{,}95$ :
\begin{itemize}[topsep=2pt]
    \item Risque officiel : $12/1\,995 = \mathbf{0{,}60\%}$ (CG + IC, conforme au protocole) ;
    \item Gain cumulé : $\approx \mathbf{330\ \text{heures}}$.
\end{itemize}
La différence quantifie le \og coût d'incertitude opérationnelle \fg{} : on ne connaît pas
en temps réel quel CG sera le dernier.
\end{tcolorbox}

\paragraph{Pistes d'amélioration de la confidence.} Pour rapprocher le résultat du sweep officiel
de la mesure oracle ($0{,}60\%$), on pourrait :
\begin{itemize}[leftmargin=*]
    \item \textbf{Combiner plusieurs signaux} : $S(30|X)$, le quantile $T_q$, le \texttt{rolling\_gap\_5},
    le rang du CG dans l'alerte ;
    \item \textbf{Entraîner un classifieur secondaire} (logistic / gradient boosting) sur la cible
    \og ce CG est-il le dernier ? \fg{} et utiliser sa proba en sortie comme \texttt{confidence} ;
    \item \textbf{Ajouter un cap dur à 30 min} : ne jamais lever avant 30 minutes même quand
    $\hat t^a_i$ est plus court (évite les incidents triviaux sur CG isolés du début d'alerte).
\end{itemize}
...
\textbf{Évaluation sur eval 2023-25 selon le protocole officiel (sweep sur $\theta$, voir
section~\ref{sec:sweep_theta}) :}
\begin{itemize}[leftmargin=*,topsep=2pt]
    \item Le risque décroît monotonement de \textbf{97,59\%} ($\theta=0$) à \textbf{3,61\%}
    ($\theta=0,95$) au fur et à mesure que l'on filtre les prédictions à faible
    $S(30 \mid X)$ ;
    \item Le seuil $R < 2\%$ du protocole officiel n'est pas atteint avec la confidence
    $S(30 \mid X)$ seule — un meilleur classifieur \og dernier CG \fg{} reste à concevoir ;
    \item En mode oracle (une prédiction par alerte au dernier CG observé), on obtient
    $R = 0{,}60\%$ et gain $\sim 330$\,h à $q = 0{,}95$, ce qui borne le potentiel du modèle.
\end{itemize}
\end{tcolorbox}
