...
\paragraph{Résultats — deux mesures complémentaires.}

Le notebook officiel propose une procédure d'évaluation avec sweep sur $\theta$. Nous présentons
deux lectures complémentaires, toutes deux avec la restriction \textbf{CG <3 km}
(les IC ne sont pas dangereux au sol). Le baseline Météorage 30 min sert de référence.

\textbf{Mesure 1 — Section 8 : 1 décision par alerte au dernier CG} (toutes les 1\,352 alertes
évaluées) :
...
0,97 & 11,8 min & 0 / 385 & 0 \% & 309 h \\
0,99 & 17,4 min & 0 / 385 & 0 \% & 269 h \\
\midrule
Baseline 30 min & — & 0 / 385 & 0 \% & 0 h \\
\bottomrule
\end{tabular}
\caption{Mesure principale : code identique à Evaluation\_databattle\_meteorage.ipynb avec
1 prédiction par alerte au dernier CG observé. \textbf{Risque CG = 0 \% par construction d'une
alerte Météorage} (aucun CG n'arrive plus de 30 min après le dernier, sinon une nouvelle alerte
démarre).}
\label{tab:section8}
\end{table}

\textbf{Mesure 2 — Sweep $\theta$ strict} : une prédiction à chaque CG ($T_q$ fixé à $q=0{,}95$),
filtrage par confidence $\geq \theta$, sélection de la plus précoce par alerte :
...
\begin{tabular}{ccccc}
\toprule
\textbf{$\theta$} & \textbf{Alertes filtrées} & \textbf{CG <3 km manqués} & \textbf{Risque CG} & \textbf{Gain cumulé} \\
\midrule
0,70 & 206 / 1\,352 & 55 / 385 & 14,29 \% & 163 h \\
...
\bottomrule
\end{tabular}
\caption{Mesure alternative : sweep strict sur $\theta$ avec prédictions multiples par alerte.
Donne un risque CG non-trivial (>0) mais moins d'alertes filtrées.}
\label{tab:sweep_strict}
\end{table}

\paragraph{Interprétation.}
\begin{itemize}[leftmargin=*]
    \item \textbf{Section 8} maximise le gain (toutes les alertes évaluées) et exploite le fait
    qu'aucun CG n'arrive après last\_CG + 30 min dans une alerte. C'est la lecture la plus
    \emph{opérationnelle} : on lève au dernier CG + $T_q$ et on est certain de n'avoir aucun
    incident CG.
    \item \textbf{Sweep $\theta$} est la lecture la plus stricte : on simule l'incertitude
    opérationnelle (on ne sait pas en temps réel quel CG sera le dernier). Le risque CG non-nul
    quantifie cette incertitude.
\end{itemize}

\begin{tcolorbox}[colback=blue!5,colframe=blue!50!black,title=\textbf{Honnêteté scientifique}]
Le \textbf{0 \% de la mesure Section 8} n'est pas un mérite du modèle — c'est une conséquence
mathématique de la définition d'une alerte Météorage. Le modèle exploite simplement cette
structure pour gagner du temps avant le timer 30 min, sans rien manquer côté CG.

Sur la \textbf{mesure sweep $\theta$ strict}, on n'atteint pas la cible officielle $R < 2\%$ :
le minimum est 2,86 \% à $\theta = 0{,}99$. Pour y aller, il faudrait améliorer la confidence
(classifieur secondaire \og dernier CG \fg{} dédié).
\end{tcolorbox}

\paragraph{Pistes d'amélioration.}
\begin{itemize}[leftmargin=*]
    \item \textbf{Classifieur secondaire \og dernier CG \fg{}} entraîné avec $S(30|X)$, $T_q$,
    \texttt{rolling\_gap\_5}, \texttt{cg\_rank} comme features. Permettrait au sweep $\theta$
    de mieux filtrer.
    \item \textbf{Cap dur à 30 min} sur les CG en début d'alerte : supprime les incidents triviaux
    sur CG isolés.
...
\textbf{Évaluation sur eval 2023-25 (critère CG <3 km, voir section~\ref{sec:sweep_theta}) :}

\textit{Mesure principale (1 décision par alerte au dernier CG, toutes les 1\,352 alertes
évaluées)} :
\begin{itemize}[leftmargin=*,topsep=2pt]
    ...
    \item Le 0 \% est une conséquence de la définition d'une alerte (aucun CG après last\_CG + 30 min).
\end{itemize}

\textit{Mesure alternative (sweep $\theta$ strict, ALL CG predictions)} :
\begin{itemize}[leftmargin=*,topsep=2pt]
    \item À $\theta = 0{,}85$ : 7,53 \% CG / 118 h sur 140 alertes filtrées.
    \item À $\theta = 0{,}99$ : 2,86 \% CG / 26 h sur 26 alertes filtrées.
